...
DirectVT Gateway is used to run DirectVT-aware applications. It is activated by the `-r gate` option.

Example (running `vtm -r term` command in DirectVT mode on top of built-in terminal of `gate` type):
```
vtm -r gate vtm -r term
```
