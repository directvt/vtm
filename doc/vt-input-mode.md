...
The `xx_chord=<HexFormString>` attributes contain the set of simultaneously pressed key IDs (e.g., KeyId0,...,KeyIdN) in ascending order and are used to track key combinations (chords). It is possible to track both chord presses (`+`, e.g., `Ctrl+F1`) and chord releases (`-`, e.g., `Ctrl-F1` or `Ctrl-Alt`):
...
