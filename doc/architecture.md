...
-------|-----------------------------|----------------------
`vtty` | `Teletype Console`          | A solid rectangular truecolor text canvas depicting a freely scrollable buffer of the text runs generated by an xterm-compatible parser from the standard output of an attached cui-application.
`term` | `Desktop Terminal`          | A derivative of `Teletype Console` with additional UI controls.
`dtvt` | `DirectVT Console`          | A solid rectangular truecolor text canvas depicting the content of an attached dtvt-application.
`xlvt` | `DirectVT Console with TTY` | A derivative of `DirectVT Console` stacked with additional `Teletype Console` as a controlling terminal. It is used for the cui-applications that forwards DirectVT traffic to standard output and requires user input via platform TTY. Depending on activity the corresponding console became active for the user.
