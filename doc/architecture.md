...
Each desktop window has a client area for the hosted object bitmap, sizing grips around the client area, a window title at the top, and a window footer at the bottom.

The desktop window can host an object instance of any registered type. The hosted object controls all parent window properties.

Registered object types:

 Type   | Name                          | Brief description
--------|-------------------------------|----------------------
`vtty`  | `Teletype Console`            | A solid rectangular truecolor text canvas depicting a freely scrollable buffer of the text runs generated by an xterm-compatible parser from the standard output of an attached console application.
`term`  | `Desktop Terminal`            | A derivative of `Teletype Console` with additional UI controls.
`shell` | `Desktop Terminal with Shell` | A copy of `Desktop Terminal` with pre-launching the default command-line shell to host the specified console application.
`dtvt`  | `DirectVT Console`            | A solid rectangular truecolor text canvas depicting the content of an attached dtvt-application, received using the DirectVT binary protocol.
`xlvt`  | `DirectVT Console with TTY`   | A derivative of `DirectVT Console` stacked with additional `Teletype Console` as a controlling terminal. It is used for the console applications that forwards DirectVT traffic to standard output and requires user input via platform TTY. Depending on activity the corresponding console became active for the user.
...
`site`  | `Desktop Region Marker`       | A transparent resizable frame for marking the specific region for quick navigation across the borderless desktop.

The desktop root after creating a new window or attaching a new user broadcasts an event to update users taskbars.
...
- Run command:
    ...
    ```bash
    vtm -r
    # The `vtm -r` option is auto converted to the `vtm -r vtty`.
    ```
...
- Run command:
    ```bash
    vtm -r vtty /path/to/console/app
    # The `vtm -r vtty` option means to run the built-in terminal without menu and bottom bar (vtty built-in terminal type).
    ```
    or
    ```bash
    ...
    # The `vtm -r vtty ...` option is used by default.
    ```
...
- Local side
    ...
    # The `ssh user@server vtm -r vtty` statement means to connect via ssh and run the built-in terminal of vtty type on the remote host.
    ```
    or
    ```bash
