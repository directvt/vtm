...
Keystrokes can be the following types
- Functional. Detected by `KeyId`.
- Printable. Detected by non-zero `UniCode`.
- Place-dependent. Detected by `ScanCode`.

Function keys always have `UniCode` set to zero, and set of `C1`, ..., `Cn` containing VT-string generated by keystroke.
...
Each key has a unique `KeyId` ID which maps to `VirtCode`, `ScanCode` and the `CtrlState` flag. The string generated by the keystroke is not included in this mapping because it depends on modifiers state and the national keyboard layout.
