...
#define MONOTTY_VER "Monotty Desktopio Preview v0.3.15"
// Autostart demo apps.
//#define DEMO
...
text cellatix_logo = ansi::bgc(greendk)   + "▀▄";
text informio_logo = ansi::bgc(magentadk) + "▀▄";
text ansiplex_logo = ansi::bgc(reddk)     + "▀▄";
...
int main(int argc, char* argv[])
{
    ...
    #pragma region samples
        ...
        text wiki00 = ansi::wrp(wrap::on).jet(bias::left).fgc(clr) + "ANSI escape code\n\n"
            ...
            + ansi::jet(bias::center).itc(true)
            + "\"ANSI code\" redirects here.\n"
            + "For other uses, see ANSI (disambiguation).\n\n"

            + ansi::jet(bias::left).itc(faux).fgc(clr) + "ANSI escape sequences" + ansi::nil()
            + " are a standard for " + ansi::fgc(clr) + "in-band signaling" + ansi::nil()
            + " to control the cursor location, color, and other options on video "
            + ansi::fgc(clr) + "text terminals" + ansi::nil() + " and "
            + ansi::fgc(clr) + "terminal emulators" + ansi::nil() + ". Certain sequences of "
            + ansi::fgc(clr) + "bytes" + ansi::nil() + ", most starting with "
            + ansi::fgc(clr) + "Esc" + ansi::nil() + " and '[', are embedded into the text, "
        ...
        text topic;
        {
            ...
            text wiki_ru = "\n\n"
                + ansi::fgc(clr) + "Управляющие символы ANSI" + ansi::nil()
                + " (англ. ANSI escape code) — символы, встраиваемые в текст, для управления форматом,"
                + " цветом и другими опциями вывода в "
                + ansi::fgc(clr) + "текстовом терминале" + ansi::nil() + "."
                + " Почти все " + ansi::fgc(clr) + "эмуляторы терминалов" + ansi::nil()
                + ", предназначенные для отображения текстового вывода с удалённого компьютера и (за исключением "
                + ansi::fgc(clr) + "Microsoft Windows" + ansi::nil() + ") для отображения текстового вывода"
                + " локального программного обеспечения, способны интерпретировать по крайней мере некоторые"
                + " управляющие последовательности ANSI.";

            text wiki_emoji = ansi::fgc(clr) + "\n\n"
                + "Emoji\n" + ansi::nil()
                + "Emoji (Japanese: 絵文字, English: /ɪˈmoʊdʒiː/; Japanese: [emodʑi];"
            ...
            +ansi::fgc(clr).wrp(wrap::on) + "\n" + "Smileys (wrap ON)\n" + ansi::nil()
                ...
                + "して"
                + ansi::fgc(clr) + "ターミナルエミュレータ" + ansi::nil()
                + "、また特にグラフィカルユーザインタフェース "
                + "(GUI) 環境で用いるものを"
                + ansi::fgc(clr) + "端末ウィンドウ" + ansi::nil()
                + "と呼ぶことがある。キャラクタユーザインタフェースを提供する。"
                + "\n\n"
                + ansi::fgc(clr) + "概要\n" + ansi::nil()
                + "端末エミュレータは、専用端末の機能を、パーソナルコンピュータ (PC) "
                + "やUnixワークステーションなどで代替するためのソフトウェア。通常はキャ"
                + "ラクタベースのビデオ端末をエミュレートするが、グラフィック端末（xterm"
                + "がTektronix 4014をエミュレートする）やプリンタのエミュレーションを行"
                + "うものもある。"
                + "\n\n"
                + "端末エミュレータを動作させるコンピュータがウィンドウシステムを搭載して"
                + "いる場合、これを利用して一つのコンピュータ上で複数の端末エミュレータを"
                + "同時に稼働させることができることが多い。これは殆どの専用端末では実現で"
                + "きない機能である。"
                + "\n\n"
                + "TCP / IPを介した端末エミュレータの接続にはSSH、Telnet、rlogin等の機"
...
utility like ctags is used to locate the definitions.
        ...
        text cellatix_text;
        {
            ...
            for (auto c = 'A'; c <= 'Z'; c++)
            {
                ...
                cellatix_text_head += ""
                    + ansi::bgc(clr - step * 0 /* 0xf0f0f0 */) + " "
                    + ansi::bgc(clr - step * 1 /* 0xededed */) + " "
                    + ansi::bgc(clr - step * 2 /* 0xeaeaea */) + c
                    + ansi::bgc(clr - step * 3 /* 0xe7e7e7 */) + " "
                            ...
                            auto label_area = item_area->template attach<ui::fork>();
                                auto mark_app = label_area->template attach<slot::_1, ui::fork>();
                                    auto mark = mark_app->template attach<slot::_1, ui::pads>(dent{ 2,1,0,0 }, dent{ 0,0,0,0 })
                                                        ->template attach<ui::item>(ansi::fgc4(0xFF00ff00) + "‣", faux);
                                ...
                                auto block = item_area->template attach<ui::fork>(axis::X);
                                    ...
                                    auto label_area = block->template attach<slot::_2, ui::pads>(dent{ 1,1,0,0 }, dent{ 0,0,0,0 });
                                        auto label = label_area->template attach<ui::item>(
                                            ansi::fgc4(0xFFffffff) + objs_desc[class_id], true, true);
                        }
                        return menuitems;
                    };
