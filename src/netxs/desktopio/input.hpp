...
namespace netxs::input
{
        ...
        static constexpr auto key_map = []
        {
            ...
            auto fill = [&](si32 KeyId, qiew codes)
            {
                while (codes)
                {
                    ...
                    //if (m[key_hash]) log("key duplicates");
                    m[key_hash] = (si16)KeyId;
                }
            };
            #define X(KeyId, Input, Name, Generic, Literal, Uc, KKPdef, KKPsuffix, KKPascii, wCtl, PhysicalCode) \
                {\
                auto codes = qiew{ PhysicalCode };\
                while (codes)\
                {\
                    auto key_hash = utf::to_int_from_hex_str(codes.pop_front(6)) >> 8;\
                    if (m[key_hash]) m[key_hash] = (si16)0xff;\
                    else m[key_hash] = (si16)KeyId;\
                }\
                }
                    //if (m[key_hash]) log("key duplicates");
    ...
    namespace key
    {
        ...
        auto xlat(si32 scancod, bool extflag, si32 keymod, si32 xlayout, si32 klid_fallback)
        {
            ...
            auto hash = input::key::key_hash(klid, scancod, extflag);
